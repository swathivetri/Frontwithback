...
selectElement.addEventListener("change", async () => {
  ...
  const expenseDetails = await axios.get(`http://13.48.56.96:3000/expense/getexpenses?param1=${pageno}&param2=${rowsize}`,{headers: {"Authorization":token}});
            showPagination(
...
function addNewExpense(e){
    ...
    console.log(expenseDetails)
    const token  = localStorage.getItem('token')
      const adddata = axios.post('http://13.48.56.96:3000/expense/addexpense',expenseDetails,  { headers: {"Authorization" : token} })
        .then((response) => {
...
window.addEventListener('DOMContentLoaded', ()=> {
    ...
    const expenseDetails = axios.get('http://13.48.56.96:3000/expense/getexpenses?param1=${pageno}&param2=${pageSize}', { headers: {"Authorization" : token} })
    .then(response => {
...
function deleteExpense(e, expenseid) {
    const token = localStorage.getItem('token')
    axios.delete(`http://13.48.56.96:3000/expense/deleteexpense/${expenseid}`,  { headers: {"Authorization" : token} }).then(() => {
...
function download() {
    ...
    axios.get('13.48.56.96:3000/expense/download', { headers: {"Authorization" : token}})
    .then((response) => {
 ...
 function downloadAllFile()
{
    try {
        ...
        const token=localStorage.getItem("token");
      axios.get("13.48.56.96:3000/expense/downloaddataAllFile",{headers: {"Authorization":token}})
       .then((response)=>{
            if(response.status===200){
...
function downloadFile(fileUrl) {
 ...
 function getProducts (page){
    ...
    const expenseDetails = axios.get(`http://13.48.56.96:3000/expense/getexpenses?param1=${page}&param2=${pageSize}`,{headers: {"Authorization":token}})
...
document.getElementById('rzp-button1').onclick = async function (e) {
    const token = localStorage.getItem('token')
    const response  = await axios.get('http://13.48.56.96:3000/purchase/premiummembership', { headers: {"Authorization" : token} });
    console.log(response);
    var options =
    {
     ...
     "handler": async function (response) {
        const res = await axios.post('http://13.48.56.96:3000/purchase/updatetransactionstatus',{
